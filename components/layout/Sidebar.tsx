...
export default function Sidebar({ isMobileOpen = false, onMobileClose }: SidebarProps) {
  ...
  return (
    <aside
      ...
      style={sidebarStyle}
      className={`group fixed inset-y-0 left-0 z-50 flex h-[calc(100vh-3.5rem)] w-[min(85vw,320px)] -translate-x-full flex-col overflow-hidden border border-white/15 bg-white/[0.1] pt-16 backdrop-blur-2xl shadow-[0_0_25px_rgba(0,0,0,0.35)] transition-transform duration-300 ease-out ${
        isMobileOpen ? "translate-x-0" : ""
      } lg:sticky lg:top-20 lg:h-[calc(100vh-5rem)] lg:w-full lg:max-w-full lg:translate-x-0 lg:rounded-3xl lg:border lg:bg-white/[0.08] lg:pt-0 lg:shadow-[0_0_20px_rgba(0,0,0,0.2)] lg:transition-[width] lg:duration-300 lg:[width:var(--sidebar-width)]`}
    >
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                key={item.href}
                href={item.href}
                data-tooltip={collapsedForLayout ? item.label : undefined}
                ...
                } ${collapsedForLayout ? "lg:justify-center" : ""}`}
      ...
      <button
        ...
        className="absolute bottom-4 left-1/2 hidden h-11 w-11 -translate-x-1/2 items-center justify-center rounded-xl border border-white/10 bg-white/5 text-text/70 transition-colors hover:bg-white/10 lg:flex"
      >
