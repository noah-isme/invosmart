  ...
  return (
    ...
    >
      ...
      <header className="relative space-y-3">
        <p className="text-xs uppercase tracking-[0.42em] text-white/50">AI invoice generator</p>
        <h1 className="text-3xl font-semibold text-white">Buat draft invoice dari perintah natural language</h1>
      ...
      <label className="relative mt-6 flex flex-col gap-3">
        <span className="text-[0.7rem] font-semibold uppercase tracking-[0.32em] text-white/55">Instruksi AI</span>
        <textarea
          value={prompt}
          onChange={(event) => onPromptChange(event.target.value)}
          placeholder="Tuliskan detail pekerjaan, nilai total, dan informasi klien..."
          className="min-h-[150px] rounded-2xl border border-white/10 bg-white/[0.05] px-4 py-3 text-sm text-white shadow-[inset_0_1px_0_rgba(255,255,255,0.1)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#6366F1]/70 focus-visible:ring-offset-2 focus-visible:ring-offset-[#0E1016]"
          disabled={loading}
      ...
      {error ? (
        ...
        <p className="mt-4 text-xs text-white/55" aria-live="polite">
      ...
      <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <motion.button
          ...
          whileTap={{ scale: 0.95 }}
          disabled={loading}
          className="gradient-button inline-flex items-center justify-center rounded-2xl px-6 py-2 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:opacity-60"
        >
